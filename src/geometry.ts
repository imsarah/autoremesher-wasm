 ...
 */
export function quadsToTriangles(quads: Uint32Array): Uint32Array {
    let triangleCount = 0;
 ...
 */
export function pairTriangleFacesIntoQuads(indices: Uint32Array): Uint32Array {
...
export function meshBoundingBox(vertices: Float32Array): BoundingBox {
...
export function boundingBoxVolume(box: BoundingBox): number {
...
export function boundingBoxDiagonal(box: BoundingBox): number {
    const dx = box.max[0] - box.min[0];
    const dy = box.max[1] - box.min[1];
    const dz = box.max[2] - box.min[2];
...
export function meshSurfaceArea(vertices: Float32Array, indices: Uint32Array): number {
    let area = 0;
    for (let i = 0; i < indices.length; i += 3) {
        const ia = indices[i] * 3;
        const ib = indices[i + 1] * 3;
 ...
 */
export function weldVerticesByPosition(
    vertices: Float32Array,
    indices: Uint32Array,
    quantize = 1e5
): { vertices: Float32Array; indices: Uint32Array; weldedFrom: number } {
 ...
 */
export function sanitizeTriangleIndices(
    vertices: Float32Array,
    indices: Uint32Array,
    areaEpsilon = 1e-18
 ...
 */
export function assessTriangleTopology(
    vertices: Float32Array,
    indices: Uint32Array
): TriangleTopologyQuality {
 ...
 */
export function extractConnectedShells(indices: Uint32Array): number[][] {
 ...
 */
export function extractManifoldShells(indices: Uint32Array): number[][] {
    return extractShellsByAdjacency(indices, /* manifoldOnly */ true);
}

function extractShellsByAdjacency(
    indices: Uint32Array,
 ...
 */
export function makeFaceWindingConsistent(indices: Uint32Array): Uint32Array {
    const faceCount = indices.length / 3;
    if (faceCount === 0)
        return indices;
    ...
    const edgeSign = (f: number, u: number, v: number): number => {
        const base = f * 3;
        for (let e = 0; e < 3; ++e) {
            const a = out[base + e];
    ...
    // undirected edge → faces that use it (and the two endpoints)
    type Link = { face: number; u: number; v: number };
    const edgeFaces = new Map<string, Link[]>();
    for (let f = 0; f < faceCount; ++f) {
        const base = f * 3;
        for (let e = 0; e < 3; ++e) {
            const a = out[base + e];
    ...
    const neighbors: Array<Array<{ face: number; u: number; v: number }>> =
 ...
 */
export function selectLargestShells(
    vertices: Float32Array,
    indices: Uint32Array,
    options: SelectShellsOptions = {}
): SelectShellsResult {
    ...
    const keptFaceSet = new Set<number>();
    ...
    for (const f of sortedFaces) {
        const base = f * 3;
        newIndices.push(
            mapVertex(consistent[base]),
            mapVertex(consistent[base + 1]),
            mapVertex(consistent[base + 2])
        );
    }
...
function indexBuffersEqual(a: Uint32Array, b: Uint32Array): boolean {
...
let decimatorReadyPromise: Promise<void> | null = null;
 ...
 */
export function ensureDecimatorReady(): Promise<void> {
    if (!decimatorReadyPromise) {
        decimatorReadyPromise = MeshoptSimplifier.ready.then(() => undefined);
    }
...
function decimateWithMeshopt(
    ...
    const hasSloppy = typeof (MeshoptSimplifier as { simplifySloppy?: unknown }).simplifySloppy
        === "function";
    ...
    const attempts: { error: number; sloppy: boolean }[] = [
    ...
    for (const { error, sloppy } of attempts) {
        try {
            ...
            let outIdx: Uint32Array | undefined;
            if (sloppy) {
                const result = (MeshoptSimplifier as {
                    simplifySloppy: (
                        i: Uint32Array,
                        v: Float32Array,
                        stride: number,
                ...
                }).simplifySloppy(src, vertices, 3, null, targetIndexCount, error);
                outIdx = result[0];
            } else {
 ...
 */
function decimateWithGridCluster(
    vertices: Float32Array,
    indices: Uint32Array,
    maxTriangles: number
): { vertices: Float32Array; indices: Uint32Array; fromTriangles: number; toTriangles: number } {
    ...
    for (let attempt = 0; attempt < 24; ++attempt) {
        ...
        if (nR > 0) {
            ...
            for (let i = 0; i < newVerts.length; i += 3) {
                const r = Math.hypot(
                    newVerts[i] - center[0],
                    newVerts[i + 1] - center[1],
                    newVerts[i + 2] - center[2]
                );
                varR += (r - avgR) * (r - avgR);
            }
 ...
 */
export function decimateToTriangleBudget(
    vertices: Float32Array,
    indices: Uint32Array,
    maxTriangles: number
): DecimateResult {
 ...
 */
export function aspectRatioNormalize(
    vertices: Float32Array,
    targetSize = 2,
    rodThreshold = 2.0
    ...
    const scale: [number, number, number] = useNonUniform
...
export function aspectRatioRestore(
    vertices: Float32Array,
    transform: Pick<AspectRatioTransform, "center" | "scale">
): Float32Array {
 ...
 */
export function assessRemeshQuality(
    sourceVertices: Float32Array,
    resultVertices: Float32Array,
    ...
    if (collapsed) {
        reasons.push(
            `collapsed bounds (diag ${diagonalRatio.toFixed(2)}, `
    ...
    if (options.requireWatertight && boundaryEdges > 0) {
        holeReasons.push(
            `mesh has holes (${boundaryEdges} boundary edges) — not enough quads to close the surface`
        );
    }
...
export function fillMeshHoles(
    ...
    const solidDirected = new Map<string, [number, number]>();
    ...
    const peekHas = (from: number): boolean => {
        const list = outgoing.get(from);
    ...
    for (const start of starts) {
        ...
        let current = start;
        const seenInLoop = new Set<number>([start]);
        let closed = false;
...
function isSafeFanHole(vertices: Float32Array, loop: number[]): boolean {
    if (loop.length > 16)
        return false;
    ...
    for (const vertex of loop) {
        ...
        const distance = Math.abs(
            (vertices[i] - cx) * nx + (vertices[i + 1] - cy) * ny + (vertices[i + 2] - cz) * nz
        );
 ...
 */
export function appendTriangleQuads(
    quads: Uint32Array,
 ...
 */
export function clampTargetTriangleCount(
    requested: number,
    area: number,
    diagonal: number,
