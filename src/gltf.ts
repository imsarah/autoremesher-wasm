...
function extractTriangles(json: GltfJson, bin: Uint8Array | null): ParsedGltf {
    const buffers: Uint8Array[] = (json.buffers ?? []).map((buffer, index) => {
        ...
        throw new Error(
            `glTF buffer ${index} references external file "${buffer.uri}"; ` +
    ...
    const result: ParsedGltf = { vertices, indices };
...
): { accessor: NonNullable<GltfJson["accessors"]>[number]; bytes: Uint8Array; stride: number } {
...
function readVec3FloatAccessor(
    json: GltfJson,
    buffers: Uint8Array[],
    accessorIndex: number
): Float32Array {
...
function tryReadVec2FloatAccessor(
    json: GltfJson,
    buffers: Uint8Array[],
    accessorIndex: number,
...
function readScalarIntAccessor(
    json: GltfJson,
    buffers: Uint8Array[],
    accessorIndex: number
): Uint32Array {
