 ...
 */
export function pairTriangleFacesIntoQuads(indices) {
...
export function meshBoundingBox(vertices) {
...
export function boundingBoxVolume(box) {
...
export function boundingBoxDiagonal(box) {
    const dx = box.max[0] - box.min[0];
    const dy = box.max[1] - box.min[1];
    const dz = box.max[2] - box.min[2];
...
export function meshSurfaceArea(vertices, indices) {
    let area = 0;
    for (let i = 0; i < indices.length; i += 3) {
        const ia = indices[i] * 3;
        const ib = indices[i + 1] * 3;
 ...
 */
export function weldVerticesByPosition(vertices, indices, quantize = 1e5) {
 ...
 */
export function sanitizeTriangleIndices(vertices, indices, areaEpsilon = 1e-18) {
    const kept = [];
    for (let i = 0; i < indices.length; i += 3) {
        const a = indices[i];
        const b = indices[i + 1];
 ...
 */
export function countBoundaryEdges(indices) {
    const edges = new Map();
    for (let i = 0; i < indices.length; i += 3) {
 ...
 */
export function assessTriangleTopology(vertices, indices) {
 ...
 */
export function extractConnectedShells(indices) {
 ...
 */
export function extractManifoldShells(indices) {
 ...
 */
export function makeFaceWindingConsistent(indices) {
    const faceCount = indices.length / 3;
    if (faceCount === 0)
        return indices;
 ...
 */
export function selectLargestShells(vertices, indices, options = {}) {
 ...
 */
export function ensureDecimatorReady() {
...
function decimateWithMeshopt(vertices, indices, maxTriangles) {
    ...
    const hasSloppy = typeof MeshoptSimplifier.simplifySloppy
        === "function";
    ...
    for (const { error, sloppy } of attempts) {
        try {
            ...
            let outIdx;
            if (sloppy) {
                const result = MeshoptSimplifier.simplifySloppy(src, vertices, 3, null, targetIndexCount, error);
                outIdx = result[0];
            }
            else {
                const result = MeshoptSimplifier.simplify(src, vertices, 3, targetIndexCount, error);
                outIdx = result[0];
            }
 ...
 */
export function decimateToTriangleBudget(vertices, indices, maxTriangles) {
    const fromTriangles = indices.length / 3;
 ...
 */
export function aspectRatioNormalize(vertices, targetSize = 2, rodThreshold = 2.0) {
    const box = meshBoundingBox(vertices);
    const extents = [
...
export function aspectRatioRestore(vertices, transform) {
 ...
 */
export function assessRemeshQuality(sourceVertices, resultVertices, resultIndices, quadCount, options = {}) {
    const reasons = [];
    const holeReasons = [];
 ...
 */
export function fillMeshHoles(vertices, indices) {
 ...
 */
export function appendTriangleQuads(quads, triangles) {
 ...
 */
export function clampTargetTriangleCount(requested, area, diagonal, inputTriangleCount) {
