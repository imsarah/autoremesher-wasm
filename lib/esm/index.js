 ...
 */
import { appendTriangleQuads, aspectRatioNormalize, aspectRatioRestore, assessRemeshQuality, assessTriangleTopology, boundingBoxDiagonal, clampTargetTriangleCount, computeVertexNormals, countBoundaryEdges, fillMeshHoles, meshBoundingBox, meshSurfaceArea, quadsToTriangles, selectLargestShells, decimateToTriangleBudget, ensureDecimatorReady, weldVerticesByPosition, } from "./geometry.js";
 ...
 */
export async function remesh(input, options = {}) {
    ...
    targetTriangleCount = clampTargetTriangleCount(targetTriangleCount, area, diagonal, inputTriangleCount);
    ...
    for (let attempt = 0; attempt < attempts.length; ++attempt) {
        ...
        try {
            ...
            let quality = assessRemeshQuality(remeshSourceVertices, restoredVertices, triangles, quadCount, { requireWatertight: false });
            ...
            if (requireWatertight) {
                ...
                if (beforeHoles > 0) {
                    ...
                    if (filled.filledTriangles.length > 0) {
                        const repairedTopology = assessTriangleTopology(restoredVertices, filled.indices);
            ...
            quality = assessRemeshQuality(remeshSourceVertices, restoredVertices, triangles, quadCount, { requireWatertight });
    ...
    if (bestHoley && requireWatertight) {
        throw new AutoRemesherError(`Remeshing left holes (${bestHoley.boundaryEdges} open edges, `
    ...
    if (bestAny && bestAny.quadCount >= 12) {
        ...
        const q = assessRemeshQuality(remeshSourceVertices, bestAny.vertices, bestAny.triangles, bestAny.quadCount, { requireWatertight: false });
    ...
    throw new AutoRemesherError(`Remeshing failed after retries (${lastQualityReasons}). `
...
function finishResult(args) {
    ...
    if (sourceUvs && options.preserveUVs !== false) {
        result.uvs = transferAttribute({ vertices: sourceVertices, indices: sourceIndices }, sourceUvs, 2, vertices);
    }
    return result;
}
function normalizeInput(input) {
    ...
    throw new AutoRemesherError("Unsupported input: pass {vertices, indices}, OBJ text, GLB/glTF data, or a BufferGeometry", -101);
}
